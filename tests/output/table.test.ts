...
import { renderPlanTable, renderTable } from "@/output/table"
...
describe("renderTable", () => {
  ...
  it("handles empty rows", () => {
    const result = renderTable(["Name", "Value"], [])
    expect(result).toContain("Name")
    expect(result).toContain("Value")
  ...
  it("renders table with wrapped content objects", () => {
    const result = renderTable(["Header"], [["plain text"]])
    expect(result).toContain("plain text")
  })

  it("renders table with multiple columns", () => {
    const result = renderTable(["A", "B", "C", "D"], [["1", "2", "3", "4"]])
...
describe("renderPlanTable", () => {
  ...
  it("returns string type", () => {
    const result = renderPlanTable([{ kind: "Monitor", key: "test", action: "NOOP", details: "—" }])
