...
function _createTestConfig(tempDir: string, currentContext = "prod") {
...
describe("config command definition", () => {
  ...
  it("use subcommand requires positional name arg", () => {
    const cmd = configCommand as unknown as {
      subCommands: { use: { args: Record<string, unknown> } }
    }
...
describe("config list command definition", () => {
  it("has list subcommand", () => {
    const cmd = configCommand as unknown as {
      subCommands: { list: { meta: Record<string, unknown>; run: unknown } }
    }
...
describe("config current command definition", () => {
  it("has current subcommand", () => {
    const cmd = configCommand as unknown as {
      subCommands: { current: { meta: Record<string, unknown>; run: unknown } }
    }
