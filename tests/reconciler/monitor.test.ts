...
describe("reconcileMonitors", () => {
  ...
  it("detects UPDATE when remote differs in a spec field", async () => {
    ...
    const manifests = [makeManifest("my-api", { spec: { name: "Updated", type: "API" as const, cronSchedule: "*/5 * * * *" } as MonitorManifest["spec"] })];
    const changes = await reconcileMonitors(api, manifests);
    expect(changes).toHaveLength(1);
    expect(changes[0]!.action).toBe("UPDATE");
  ...
  it("filters by tag option", async () => {
    const api = mockMonitorsApi([makeMonitor("api-a", { id: 10 }), makeMonitor("api-b", { id: 20 })]);
  ...
  it("handles mix of CREATE, UPDATE, DELETE", async () => {
    ...
    const manifests = [
      makeManifest("exists", { spec: { name: "Exists", type: "API" as const } as MonitorManifest["spec"] }),
      makeManifest("changed", { spec: { name: "New Name", type: "API" as const } as MonitorManifest["spec"] }),
      makeManifest("new"),
    ];
