...
import { reconcile, plan, loadStateFile, saveStateFile } from "@/reconciler/engine";
...
function createMockKy(overrides: {
...
describe("engine reconcile", () => {
  ...
  it("reconcile creates new monitors", async () => {
    ...
    writeManifest(dir, "monitors.yaml",
      `kind: Monitor
metadata:
  tag: my-api
spec:
  ...
  it("reconcile respects kind filter", async () => {
    ...
    writeManifest(dir, "trigger.yaml",
      `kind: AlertTrigger
metadata:
  name: ops-slack
spec:
  ...
  webhookUrl: https://hooks.slack.com/test`);
    writeManifest(dir, "monitor.yaml",
      `kind: Monitor
metadata:
  tag: other
spec:
